...
class Index(object):
    ...
    def compute_doc_lengths(self, index):
        ...
        5.0
        """
        doc_id_len = defaultdict(int)
        #In this loop will calculate sum(w_i**2) for all docs
        for each_list in index.values():
            for each_doc in each_list:
                doc_id_len[each_doc[0]] += (each_doc[1] ** 2)

        for key in doc_id_len:
            doc_id_len[key] = sqrt(doc_id_len[key])
    ...
    def create_champion_index(self, index, threshold=10):
        ...
        for key,value in index.items():
            champion_index[key]=sorted(index[key], key=lambda item: item[1],reverse=True)[0:threshold]
    ...
    def search_by_cosine(self, query_vector, index, doc_lengths):
        ...
        return sorted(tempDict.items(), key = lambda key: key[1], reverse = True)
    ...
    def search(self, query, use_champions=False):
        ...
        vector = self.query_to_vector(tokens)
